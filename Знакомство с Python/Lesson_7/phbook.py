...
class Phonebook:
    ...
    @sql_cursor
    def add_contact(self, cursor):

        print("\nСоздание нового контакта .........................")
        name = input("Введите имя нового контакта: ")
        surname = input(f"Введите фамилию нового контакта {name}: ")
        phone = int(input(
            f"Введите номер нового контакта{name, surname} (ТОЛЬКО ЦИФРЫ: БЕЗ СИМВОЛОВ ' ' и '+'): "))
    ...
    def print_book(self, cursor):
        ...
        records = cursor.fetchall()
        print("Всего номеров:  ", len(records), end="\n\n")
        for row in records:
            print(
                f"ID КОНТАКТА: {row[0]}    Имя: {row[1]} {row[2]}    Номер: {row[5]}", end="\n\n")

    @sql_cursor
    def del_contact(self, cursor):
        user_id = int(input("Введите ID контакта, Которого хотите удалить: "))
