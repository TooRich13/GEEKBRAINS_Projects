...
{
    int count = 0;
    List <int> tags = new List<int>() ; // Лист с индексами, которые будут подчёркнуты при выводе
    for (int i = 0; i < array.Length; i++)
    {
        if (array[i] % 2 == 0) 
        {
            count++;
            tags.Add(i); // Добавляем в лист тэгов индексы значений, удволетворяющих условия
        }
    }
